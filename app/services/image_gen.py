...
class ImageGenService:
    ...
    async def run(
        ...
        all_images = await _build_image_entries(
            results,
            output_dir,
            task_id,
            timestamp,
            download_proxy=(settings.GPT_PROXY or None) if model.startswith("gpt-") else None,
        )
...
async def _download_image(url: str, output_dir: Path, filename: str, proxy: Optional[str] = None) -> Path:
    """下载图片到本地，返回保存路径"""
    filepath = output_dir / filename
    async with httpx.AsyncClient(timeout=60.0, trust_env=False, proxy=proxy) as client:
...
async def _build_image_entries(
    ...
    for r in results:
        ...
        if r.get("url"):
            if r["url"].startswith("/output/"):
                # 本地已落盘（如 GPT b64_json 场景），跳过下载
                img_info["local_path"] = str(from_output_url(r["url"]))
            else:
                try:

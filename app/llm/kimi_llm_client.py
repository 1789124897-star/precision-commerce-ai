...
class KimiClient(BaseMultimodalClient):
    ...
    def __init__(self) -> None:
        ...
        if not self._api_key:
            raise AppException("未配置 KIMI_API_KEY，请在 .env 中设置")
        if not self._base_url:
            raise AppException("未配置 KIMI_BASE_URL，请在 .env 中设置")
        if not self._model:
            raise AppException("未配置 KIMI_MODEL，请在 .env 中设置")
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        t0 = time.monotonic()
        logger.info("Kimi 多模态请求开始 model=%s images=%d", self._model, len(image_data_urls))
        data = await post_with_retry(self._base_url, payload, headers=self._headers, timeout=180.0)
        elapsed = time.monotonic() - t0
        logger.info("Kimi 多模态请求完成 耗时=%.1fs", elapsed)

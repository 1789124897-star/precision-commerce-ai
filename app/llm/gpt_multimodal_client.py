...
class GptMultimodalClient(BaseMultimodalClient):
    ...
    def __init__(self) -> None:
        ...
        self._base_url = settings.GPT_BASE_URL
        self._model = settings.GPT_MODEL
        if not self._api_key:
            raise AppException("未配置 GPT_API_KEY，请在 .env 中设置")
        if not self._base_url:
            raise AppException("未配置 GPT_BASE_URL，请在 .env 中设置")
        if not self._model:
            raise AppException("未配置 GPT_MODEL，请在 .env 中设置")
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        t0 = time.monotonic()
        logger.info("GPT 多模态请求开始 model=%s images=%d", self._model, len(image_data_urls))
        ...
        logger.info("GPT 多模态请求完成 耗时=%.1fs", elapsed)
